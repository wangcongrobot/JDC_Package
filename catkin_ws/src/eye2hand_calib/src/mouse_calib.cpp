...
private:
    ...
    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image,
            ...
            geometry_msgs::PointStamped> ApproxmateSyncPolicy;
...
private:
    ...
    cv::Vec3f rotationMatrixToEulerAngles(cv::Mat &R)
    {
        ...
        else
        {
            ...
            y = atan2(-R.at<double>(2,0), sy); z = 0;
        }
    ...
    void start()
    {
        ...
        std::string topicCameraInfo = topicColor.substr(0,
        ...
        subImageDepth = new image_transport::SubscriberFilter(it, topicDepth,
                queueSize, hints);
        subCameraInfo = new message_filters::Subscriber<
            sensor_msgs::CameraInfo>(nh, topicCameraInfo, queueSize);
            ...
            const geometry_msgs::PointStamped::ConstPtr handPoint)
    {
        ...
        readCameraInfo(cameraInfo, cameraIntrinsicMatrix,
    ...
    void imageViewer()
    {
        ...
        {
            // std::cout << "updateImage: " << updateImage << std::endl;
            // std::cout << "updateHand: " << updateHand << std::endl;
            if(updateImage && updateHand)
            {
                ...
                cv::putText(color, oss.str(), pos, font, sizeText, colorText,
                        lineText, CV_AA);
	        temp = std::to_string(mousePoint.x) + ", " + std::to_string(mousePoint.y);
                // before show
                cv::putText(color, temp, mousePoint, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255, 0));
                cv::circle(color, mousePoint, 2, cvScalar(255, 0, 0, 0), CV_FILLED, CV_AA, 0);
    ...
    {
        Calibrator* temp = reinterpret_cast<Calibrator*>(userdata);
    ...
    void on_Mouse(int event, int x, int y)
    {
        switch(event)
        {
            case CV_EVENT_LBUTTONDOWN:
                detected = true;
                lock.lock();
                mousePoint = cv::Point2f(x, y);
                imagePoint = cv::Point2f(x, y);
                std::cout << "click on Point: " << x << ", " << y << "." << std::endl;
                lock.unlock();
                savePoints();
                color_copy = color.clone();
                cv::circle(color_copy, cv::Point(x, y), 2, cvScalar(255, 0, 0, 0), CV_FILLED, CV_AA, 0);
                cv::putText(color_copy, std::to_string(x) + ", " + std::to_string(y), cv::Point(x, y), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255, 0));
                cv::imshow("Copy Viewer", color_copy);
                break;
	   ...
	   //      // std::string temp = std::to_string(pt.x) + ", " + std::to_string(pt.y);
           //      // cv::putText(color, temp, pt, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255, 0));
        ...
        const
    {
        ...
        // scale = 1000.0;
        // fx = cameraIntrinsicMatrix.ptr<double>(0)[0];
        // fy = cameraIntrinsicMatrix.ptr<double>(1)[1];
        // cx = cameraIntrinsicMatrix.ptr<double>(0)[2];
        // cy = cameraIntrinsicMatrix.ptr<double>(1)[2];
    }
    ...
    void savePoints()
    {
        ...
        if(detected)
        {
            ...
            std::cout << "hond point: " << handPoint << std::endl;
            imagePoints2D.push_back(imagePoint);
            ROS_INFO_STREAM("a new point is saved!");
            detected = false;
        }
    ...
    void removePoints()
    {
        ROS_INFO_STREAM("remove method");
        handPoints3D.pop_back();
        imagePoints2D.pop_back();
        ROS_INFO_STREAM("last point is deleted!");
    }
    ...
    void calibrate()
    {
        ...
        cv::FileStorage fs2("/home/robot/catkin_ws/src/kinect_detect/calib/external-calib-results.yaml", cv::FileStorage::WRITE);
        fs2 << "T_hand2eye" << T_hand2eye;
        fs2 << "T_eye2hand" << T_eye2hand;
        fs2.release();

        cv::Vec3f eulerAngle = rotationMatrixToEulerAngles(rotationMat);

        std::cout << "hand2eye eular angle: " << eulerAngle << std::endl;
    ...
    void validate()
    {
        ...
        cv::Mat homoHandPoint = cv::Mat(3, 1, CV_64F);
        ...
        cv::Mat homoHandPoint_in_eye;
        cv::projectPoints(homoHandPoint, rvec, tvec, cameraIntrinsicMatrix, cameraDistortionCoefficients,
                          homoHandPoint_in_eye);
        ...
        int u = homoHandPoint_in_eye.ptr<double>(0)[0];
        int v = homoHandPoint_in_eye.ptr<double>(1)[0];
