/******************************************************************************
 * This is the test file to varify the whole process of JD challenge.
...
//globals
int error_no=0;
// commment next list, cos vector
// int row[20]={0},column[20]={0};
vector<int> object_list;
const string object_types[] = {"book", "toothbrush", "can", "strips",
    "chips", "oreo", "pacific", "shampoo", "tissue", "sausage", "toothpaste",
    "teether", "milk", "jelly"};

vector<int> destination_list;
const string destination_points[] = {"shelf_1_1", "shelf_1_2", "table_1_1",
    "table_1_2"};
int obj_num=0;
int loop_sys_cnt=0;
int obj_cnt=0;
...
int main(int argc,char **argv)
{
    ...
    ifstream object_type_file("/home/robot/tasks.txt");
    string temp;
    if(!object_type_file.is_open())
    {
        ROS_ERROR("Failed to read tasks.");
        return -1;
    }


    int obj;
    while(getline(object_type_file,temp))
    {
        sscanf(temp.c_str(), "%d", &obj);
    ...
    cout<<obj_num<<"objects need to fetch, they are :"<<endl;
    for(vector<int>::iterator iter = object_list.begin(); iter != object_list.end(); iter++)
        cout << *iter <<" : "<< object_types[*iter]<<endl;
