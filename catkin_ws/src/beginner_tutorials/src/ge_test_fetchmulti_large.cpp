...
const double waittime   = 15.6;
const double time_limit = 330.0;
...
vector<Target_Pose> detected_targets;
...
bool nav_avoidance_flag;  // data[0] = 7, data[1] = 7
...
void notice_pub_sub::notice_msgCallback(const id_data_msgs::ID_Data::ConstPtr &notice_msg)
{
    ...
    if (notice_message.id == 3 && notice_message.data[0] == 7 && notice_message.data[7])
    {
        nav_avoidance_flag = true;
        nav_finished_flag = true;
    }
	...
	if (notice_message.id == 3 && notice_message.data[0] == 15) // kinect scan finished flag
	{
        ...
        target_pose.class_id = notice_message.data[1];
        target_pose.x = notice_message.data[2];
        target_pose.y = notice_message.data[3];
        target_pose.z = notice_message.data[4];
        detected_targets.push_back(target_pose);
	}
	...
	if (notice_message.id == 4 && notice_message.data[0] == 15) // arm fetch finished flag
	{
        ...
        finished_object_list.insert(notice_message.data[1]);
...
int main(int argc, char **argv)
{
	...
	// for (int i = 4; i <= place_length + 3; i++) {
    ...
    bool object_finished_flag = false;
    ...
    vector<int> place_sequence = {5, 6, 7, 8, 11, 12, 13, 15, 16, 17, 18, 21, 22, 23};
    vector<int> place_group_1  = {5, 6, 7, 8};
    vector<int> place_group_2  = {11, 12, 13};
    vector<int> place_group_3  = {15, 16, 17, 18};
    vector<int> place_group_4  = {21, 22, 23};

    for (int i_place = 0; i_place != place_sequence.size(); i_place ++)
    {
        int place = place_sequence[i_place];
        ...
        if(nav_avoidance_flag)
        {
            for(vector<int>::iterator it_place = find(place_group_1.begin(), place_group_1.end(), place);
                    it_place != place_group_1.end(); it_place ++)
            {
                place_sequence.erase(find(place_sequence.begin(), place_sequence.end(), *it_place));
                place_sequence.push_back(*it_place);
            }
            for(vector<int>::iterator it_place = find(place_group_2.begin(), place_group_2.end(), place);
                    it_place != place_group_2.end(); it_place ++)
            {
                place_sequence.erase(find(place_sequence.begin(), place_sequence.end(), *it_place));
                place_sequence.push_back(*it_place);
            }

            for(vector<int>::iterator it_place = find(place_group_3.begin(), place_group_3.end(), place);
                    it_place != place_group_3.end(); it_place ++)
            {
                place_sequence.erase(find(place_sequence.begin(), place_sequence.end(), *it_place));
                place_sequence.push_back(*it_place);
            }

            for(vector<int>::iterator it_place = find(place_group_4.begin(), place_group_4.end(), place);
                    it_place != place_group_4.end(); it_place ++)
            {
                place_sequence.erase(find(place_sequence.begin(), place_sequence.end(), *it_place));
                place_sequence.push_back(*it_place);
            }
        ...
        // if(! ((place >= 5 && place <= 8) || (place >= 11 && place <= 13) ||
        //         (place >= 15 && place <= 18) || (place >= 21 && place <= 23)))
        //     continue;
        ...
        for(set<int>::iterator it = object_list.begin(); it != object_list.end(); it++)
        {
            ...
            if (finished_object_list.find(object) != finished_object_list.end())
            {
                ROS_INFO_STREAM("Object: " << object << " has been fetched.");
                continue;
            }
            ...
            // if (finished_object_list.size() == 10)
            // {
            //     ROS_INFO("all objects have been fetched.");
            //     object_finished_flag = true;
            //     break;
            // }
        }

        if (time_up_flag || object_finished_flag)
        {
            ROS_INFO("Enter special cases");
            break;
        }
        ...
        int object_count = detected_targets.size();
        for(vector<Target_Pose>::iterator it = detected_targets.begin(); it != detected_targets.end();
                it++)
        {
            ...
            notice_data.id      = 4;
            notice_data.data[0] = 1;
            notice_data.data[1] = it -> class_id;
            notice_data.data[2] = it -> x;
            notice_data.data[3] = it -> y;
            notice_data.data[4] = it -> z;
            notice_data.data[5] = object_count;
            notice_data.data[6] = place;
            error_index =
    ...
    if(time_up_flag || object_finished_flag)
    {
        ROS_INFO("[Special cases]: Adjust dashgo pose to go to the release place!");
        notice_data_clear(&notice_data);
        notice_data.id      = 2;
        notice_data.data[0] = 6;
        notice_data.data[1] = 6;
        error_index =
            main_MsgConform_ActFinishedWait(&notice_data, &nav_msg_rec_flag,
                    &nav_finished_flag, &notice_test);
        error_deal(error_index);
    }
	...
	while ((ros::Time::now() - start).toSec() <= waittime)
        ros::Duration(0.1).sleep();
    //
	// 4,inform dashgo move to the object release position.
	ROS_INFO("4,inform dashgo move to the object release position.");
