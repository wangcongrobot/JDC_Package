...
public:
    ...
    void bhand_move_position(float base,float f1,float f2,float f3,int delay)
    {
        ...
        msg_state.velocity={0.05,0.05,0.05,0.05};
        msg_state.effort={0,0.3,0.3,0.3};
...
int main(int argc,char **argv)
{
    ...
    bhand_pose_pre_catch.Set(0,0.5,0.5,0.5,1,0);
    printf("Pre-catch pose have achieved!\n");

    notice_pub_sub notice_test;
    id_data_msgs::ID_Data notice_data_pub;
    float base,f1,f2,f3,count;
    while(ros::ok())
    {
        if(key_control_value!=0)
        {
            f1+=key_control_value;
            f2+=key_control_value;
            f3+=key_control_value;
            bhand_pose_pre_catch.Set(0,f1,f2,f3,1,count++);
        }
        if((num_count++)%100==0)
        {
            ROS_INFO("base %4f,f1 %4f,f2 %4f,f3 %4f",bhandpose1.rec_base,bhandpose1.rec_f1,bhandpose1.rec_f2,bhandpose1.rec_f3);
        }
        key_control_value=0;
        ...
        if((bhandpose1.rec_f1<=2.2 && bhandpose1.rec_f1>=0)&&(bhandpose1.rec_f2<=2.2 && bhandpose1.rec_f2>=0)&&(bhandpose1.rec_f3<=2.2 && bhandpose1.rec_f3>=0))
        {
            if(close_hand_flag)
            {
                ...
                bhandpose1.bhand_move_position(0,1.65,1.65,1.65,1);
                sleep(5);
            ...
            {
                close_hand_flag=false;
                bhandpose1.bhand_move_position(0,0.5,0.5,0.5,2);
                sleep(5);
                notice_data_clear(&notice_data_pub);
                notice_data_pub.id=1;
                notice_data_pub.data[0]=2;
        ...
        else
        {
            ...
            sleep(3);
            ROS_INFO("bhand attempt to recover from exception...");
            //bhand_pose_pre_catch.Set(0,0.5,0.5,0.5,1,0);
        }
